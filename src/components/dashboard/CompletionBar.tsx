...
export function CompletionBar() {
  const { stats, productivityScore } = useTasks();
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl glass-card px-5 py-4 space-y-2 card-hover-lift glow-primary border-0"
    >
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold text-foreground">Overall Completion</p>
          <p className="text-xs text-muted-foreground mt-0.5">
            {stats.completedToday} of {stats.total} tasks done today · Score: {productivityScore}/100
          </p>
        </div>
      ...
      <div className="h-3 rounded-full bg-muted overflow-hidden">
        <div className={cn("h-full rounded-full completion-bar-fill", barColor)} style={{ width: `${pct}%` }} />
      </div>
